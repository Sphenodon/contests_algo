...
for (let date of arr){
    let date_arr = date.split(':').map((x)=> parseInt(x, 10))
    dates.push(date_arr[0]*3600 + date_arr[1]*60 + date_arr[2])
}

if (dates[0] > dates[2]) {
    dates[2] += 24*3600
}
let total_seconds = (dates[2]-dates[0])/2+dates[1]

answer = new Date(total_seconds*1000 + 500).toUTCString().split(/ /)[4]
