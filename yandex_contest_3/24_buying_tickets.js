...
dp[0] = 0
let arr_buyers = []
arr_buyers[-2] = [Infinity,Infinity,Infinity]
arr_buyers[-1] = [Infinity,Infinity,Infinity]
arr_buyers[0] = [Infinity,Infinity,Infinity]
for (let i = 1; i<=buyers; i++){
    let buyer = arr.shift()
    arr_buyers[i] = buyer.split(' ').map((x)=> parseInt(x))
    dp[i] = Math.min(dp[i-1] + arr_buyers[i][0], dp[i-2] + arr_buyers[i-1][1], dp[i-3] + arr_buyers[i-2][2])
}

process.stdout.write(dp[buyers].toString())
process.exit()
