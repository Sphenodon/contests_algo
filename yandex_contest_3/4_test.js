...
let variants = arr.shift()
let row = arr.shift()
let place = arr.shift()
let row_upper = 0
let row_lower = 0
let place_answer = 0
function find_position(row, place){
    if (place === 1){
        row_upper = row - Math.floor((variants+1)/2)
        row_lower = row + Math.floor(variants/2)
        place_answer = 1
    }
    if (place === 2){
        row_upper = row - Math.floor(variants/2)
        row_lower = row + Math.floor((variants+1)/2)
        place_answer = 2
    }
    if (variants%2 === 1){
        place_answer = place === 2 ? 1 : 2
    }
    if (students%2===1 && (row*2%variants===place || row*2%variants-1===place) && row*2+1===students){
        return [Math.floor((students+1)/2), place]
    }
    if (row_lower*2-2+place_answer <= students && row_upper > 0){
        let row_r = row - row_upper < row_lower - row ? row_upper : row_lower
        return [row_r, place_answer]
    }
    if (row_lower*2-2+place_answer <= students){
        return [row_lower, place_answer]
    }
    if (row_upper > 0) {
        return [row_upper, place_answer]
    }

    return -1
}

process.stdout.write(find_position(row, place).toString().replaceAll(',', ' '))
process.exit()
